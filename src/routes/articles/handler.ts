...
import { articleTags } from '../../../schema/articleTags';
...
export const getMyArticles = async (request: Request, h: ResponseToolkit) => {
  const db = request.server.app.db;
...
export const getArticleDetail = async (
  ...
  if (!article) return h.response({ error: 'Not found' }).code(404);
  ...
  const [saved] = await db
    ...
    .from(articleSaves)
    .where(
      and(eq(articleSaves.articleId, id), eq(articleSaves.userId, userId))
    );
  ...
  const [vote] = await db
    ...
    .from(articleVotes)
    .where(
      and(eq(articleVotes.articleId, id), eq(articleVotes.userId, userId))
    );
...
export const searchArticles = async (request: Request, h: ResponseToolkit) => {
  const db = request.server.app.db;
