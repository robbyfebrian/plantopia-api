...
// Overview dashboard
export const getOverview = async (request, h) => {
...
export const getPlantCollection = async (request, h) => {
    const db = request.server.app.db;
    // Ensure user is authenticated
    const userId = getUserIdFromRequest(request);
    if (!userId)
...
export const getDiseaseJournal = async (request, h) => {
    const db = request.server.app.db;
    // Ensure user is authenticated
    const userId = getUserIdFromRequest(request);
    if (!userId)
...
export const getAchievementJournal = async (request, h) => {
    const db = request.server.app.db;
    // Ensure user is authenticated
    const userId = getUserIdFromRequest(request);
    if (!userId)
...
export const getMissionJournal = async (request, h) => {
    const db = request.server.app.db;
    // Ensure user is authenticated
    const userId = getUserIdFromRequest(request);
    if (!userId)
